		...
		echo "<div class='artikel'>
						...
						while ($k = mysql_fetch_array($mapel)){
								echo "<tr>
										...
										<td>$k[batas_waktu]</td>
										<td width='80px'><a href='index.php?page=laporannilai&aksii=jawaban&id=$k[id_tugas]'>Lihat Nilai</a></td>
										</tr>";
							$no++;
						}	
		echo "</table></div>";	
			...
			$kel = mysql_fetch_array(mysql_query("SELECT * FROM tbl_tugas a JOIN tbl_kelas b ON a.kd_kelas=b.kd_kelas where id_tugas='$_GET[id]'"));
				...
				echo "<div class='artikel'>
							<h1>Semua Data Siswa Kelas $kel[nm_kelas] - Tugas $_GET[id]</h1>
						<table width=100% border=1>
									<tr bgcolor=#cecece>
										<th width='50px'>No</th>
						...
						$no = 1;
						while ($k = mysql_fetch_array($mapel)){
							$n = mysql_fetch_array(mysql_query("SELECT * FROM tbl_nilai_tugas where id_tugas='$_GET[id]' AND no_induk='$k[no_induk]'"));
							$nob = mysql_fetch_array(mysql_query("SELECT count(*) as benar FROM `tbl_jawaban_objektif` a JOIN tbl_pertanyaan_objektif b ON a.id_pertanyaan_objektif=b.id_pertanyaan_objektif where a.no_induk='$k[no_induk]' AND b.id_tugas='$_GET[id]' AND a.jawaban=b.kunci_jawaban"));
							$noj = mysql_fetch_array(mysql_query("SELECT count(*) as jumlah FROM `tbl_jawaban_objektif` a JOIN tbl_pertanyaan_objektif b ON a.id_pertanyaan_objektif=b.id_pertanyaan_objektif where a.no_induk='$k[no_induk]' AND b.id_tugas='$_GET[id]'"));	
